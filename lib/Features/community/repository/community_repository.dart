import 'package:campus_connect/core/constants/firebase_constants.dart';
import 'package:campus_connect/core/provider/firebase_providers.dart';
import 'package:campus_connect/core/type_defs.dart';
...
class CommunityRepository {
  ...
  FutureVoid createCommunity(Community community) async {
    try {
      var communityDoc = await _communities.doc(community.name).get();
      if (communityDoc.exists) {
        throw 'Group with the same name already exists!';
      }
  ...
  Stream<List<Post>> getCommunityPosts(String name) {
    return _posts.where('groupName', isEqualTo: name).orderBy('createdAt', descending: true).snapshots().map(
          (event) => event.docs
              .map(
                (e) => Post.fromMap(
